...
> Not: R2 upload temel akışı eklendi (`POST /v1/media/upload-url`, `POST /v1/media/finalize`, `media_assets` migration). Finalize sırasında R2 HEAD doğrulaması, Cloudflare Images URL import ve Cloudflare Stream `/stream/copy` ingest eklendi. `DELETE /v1/media/:id` R2 + Cloudflare Images/Stream delete çağrılarını yapıyor. Cloudflare Stream webhook imza doğrulama + ready/failed status update eklendi. Cloudflare Images varyant isimleri `GET /v1/config` kontratına sabitlendi. Retention cleanup helper'ları eklendi; Trigger.dev schedule bağlama işi açık.
