...
- [ ] `AppAvatar` komponenti: avatar + fallback initials + `PresenceDot` slot'u
- [ ] `MediaTile` komponenti: image/video thumbnail, loading, error, retry
- [ ] `UserListTileV2`: avatar, display name, username, presence, takip butonu
- [ ] `AsyncPagedList` helper: cursor pagination ekranlarında ortak kullanım
- [ ] `V2EmptyState` ve `V2ErrorState`: feed/story/follow listelerinde ortak görünüm
