...
class _RoomChatScreenState extends ConsumerState<RoomChatScreen> {
  ...
  Future<void> _send() async {
    ...
    _controller.clear();
    await ref.read(roomChatProvider(widget.room).notifier).send(body);
  }

  @override
  Widget build(BuildContext context) {
    final chat = ref.watch(roomChatProvider(widget.room));

    ref.listen(roomChatProvider(widget.room), (previous, next) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(widget.room.displayTitle)),
      body: SafeArea(
        child: Column(
          children: [
            Expanded(
            ...
            _RoomComposer(controller: _controller, onSend: _send),
          ],
        ),
      ),
    );
